...
def rmFromSEUnwantedFiles():
    output = commands.getoutput("lcg-ls srm://polgrid4.in2p3.fr:8446/srm/managerv2?SFN=/dpm/in2p3.fr/home/cms/trivcat/store/user/inaranjo/"+sys.argv[1])

    outFiles = re.split(r'\n',output)
    
    for name in outFiles:
        if re.search("Tree_7TeV_DATA",name)==None:
            continue
        splitname = re.split(r'/',name)
        ...
        try:
            ...
            found = 0
            for line in lines:
                if re.search("LFN:",line)!=None and re.search("Tree_7TeV_DATA",line)!=None and found==0:
                    found = 1
                    ...
                    if re.search(jobIDsplit[ len(jobIDsplit)-1],linesplit[len(linesplit) -1])!=None:
                        ...
                        rm = "rfrm /dpm/in2p3.fr/home/cms/trivcat/store/user/inaranjo/"+sys.argv[1]+filename
                        print rm
                        os.system(rm)
