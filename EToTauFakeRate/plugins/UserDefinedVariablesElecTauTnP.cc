...
class UserDefinedVariablesElecTauTnP : public edm::EDProducer {
public:
  explicit UserDefinedVariablesElecTauTnP(const edm::ParameterSet & iConfig);
  virtual ~UserDefinedVariablesElecTauTnP() ;
...
UserDefinedVariablesElecTauTnP::UserDefinedVariablesElecTauTnP(const edm::ParameterSet & iConfig) :
...
UserDefinedVariablesElecTauTnP::~UserDefinedVariablesElecTauTnP()
{

}

void UserDefinedVariablesElecTauTnP::produce(edm::Event & iEvent, const edm::EventSetup & iSetup) {
  ...
  Handle<View<reco::Candidate> > tausHandle;
  iEvent.getByLabel(srcTaus_, tausHandle);
  ...
  ////////////////////////////////////////////////////////////////
	...
	for(unsigned int k =0; k < (aObj->pathNames()).size() ; k++){
	  if (Geom::deltaR( aObj->triggerObject().p4(), myElectron->p4() ) < 0.3 && (aObj->pathNames())[k].find(triggerPathName) != std::string::npos) {
	    ...
	    matched = true;
	    if (debug_)cout<<"Trigger Matched !"<<endl;
	  }
	}
    ...
    if(matched) HLTEle27Match_.push_back(1);
    else HLTEle27Match_.push_back(0);
    

    if(debug_){
      if(matched) cout << "Electron matched within dR=0.3 with trigger object passing path " << MatchedtriggerPathName << endl;
...
void
UserDefinedVariablesElecTauTnP::beginJob()
{
...
DEFINE_FWK_MODULE(UserDefinedVariablesElecTauTnP);
