...
double AntiElectronIDMVA2::MVAValue(const reco::PFTauRef& thePFTauRef,
				    const reco::GsfElectronRef& theGsfEleRef
				    )
{
  ...
  if (deltaR((*thePFTauRef).eta(),(*thePFTauRef).phi(),(*theGsfEleRef).eta(),(*theGsfEleRef).phi())<0.3){
    std::cout << "Electron and Tau not matched.\n"; 
    return -999;
  }
  ...
  Tau_AbsEta_ = TMath::Abs((*thePFTauRef).eta()) ;
  Tau_Pt_ = (*thePFTauRef).pt();
  Tau_HasGsf_ = ((*thePFTauRef).leadPFChargedHadrCand()->gsfTrackRef()).isNonnull();
  Tau_EmFraction_ = TMath::Max((*thePFTauRef).emFraction(),float(0.0)); 
  Tau_NumChargedCands_ = (*thePFTauRef).signalPFChargedHadrCands().size();
  Tau_NumGammaCands_ = (*thePFTauRef).signalPFGammaCands().size();
  Tau_HadrHoP_ = (*thePFTauRef).leadPFChargedHadrCand()->hcalEnergy()/(*thePFTauRef).leadPFChargedHadrCand()->p();
  Tau_HadrEoP_ = (*thePFTauRef).leadPFChargedHadrCand()->ecalEnergy()/(*thePFTauRef).leadPFChargedHadrCand()->p();
  Tau_VisMass_ = (*thePFTauRef).mass(); 
  Tau_HadrMva_ = TMath::Max((*thePFTauRef).electronPreIDOutput(),float(-1.0)); 
  ...
  Elec_AbsEta_ = TMath::Abs((*theGsfEleRef).eta());
  Elec_Pt_ = (*theGsfEleRef).pt();
  ...
  reco::SuperClusterRef pfSuperCluster = (*theGsfEleRef).pflowSuperCluster();
  if(pfSuperCluster.isNonnull() && pfSuperCluster.isAvailable()){
    ...
    Elec_Pin_ = TMath::Sqrt((*theGsfEleRef).trackMomentumAtVtx().Mag2());
    Elec_Pout_ = TMath::Sqrt((*theGsfEleRef).trackMomentumOut().Mag2()); 
    Elec_EtotOverPin_ = (Elec_Ee_+Elec_Egamma_)/Elec_Pin_;
    Elec_EgammaOverPdif_ = Elec_Egamma_/(Elec_Pin_-Elec_Pout_);
  }

  Elec_EarlyBrem_ = (*theGsfEleRef).mvaInput().earlyBrem;
  Elec_LateBrem_= (*theGsfEleRef).mvaInput().lateBrem;
  Elec_Fbrem_ = (*theGsfEleRef).fbrem();

  //Variables related to the CtfTrack
  Elec_Chi2KF_ = -99;
  Elec_NumHits_ = -99;
  if ((*theGsfEleRef).closestCtfTrackRef().isNonnull()){
    Elec_Chi2KF_ = (*theGsfEleRef).closestCtfTrackRef()->normalizedChi2();
    Elec_NumHits_ = (*theGsfEleRef).closestCtfTrackRef()->numberOfValidHits();
  }
  ...
  if((*theGsfEleRef).gsfTrack().isNonnull()){
    Elec_Chi2GSF_ = (*theGsfEleRef).gsfTrack()->normalizedChi2();
    Elec_GSFTrackResol_ = (*theGsfEleRef).gsfTrack()->ptError()/(*theGsfEleRef).gsfTrack()->pt();
    Elec_GSFTracklnPt_ = log((*theGsfEleRef).gsfTrack()->pt())*TMath::Ln10();
    Elec_GSFTrackEta_ = (*theGsfEleRef).gsfTrack()->eta();
  }


  if((*thePFTauRef).signalPFGammaCands().size()==0){
    if(TMath::Abs((*thePFTauRef).eta())<1.5) 
      mva = fTMVAReader_[0]->EvaluateMVA( methodName_ );
    else  
      mva = fTMVAReader_[4]->EvaluateMVA( methodName_ );
  }
  else if((*thePFTauRef).signalPFGammaCands().size()>0 && !((*thePFTauRef).leadPFChargedHadrCand()->gsfTrackRef()).isNonnull()){
    if(TMath::Abs((*thePFTauRef).eta())<1.5) 
      mva = fTMVAReader_[1]->EvaluateMVA( methodName_ );
    else  
      mva = fTMVAReader_[5]->EvaluateMVA( methodName_ );
  }
  else if((*thePFTauRef).signalPFGammaCands().size()>0 && ((*thePFTauRef).leadPFChargedHadrCand()->gsfTrackRef()).isNonnull() && TMath::Max((*theGsfEleRef).mvaOutput().mva,float(-1.0))<-0.1){
    if(TMath::Abs((*thePFTauRef).eta())<1.5) 
      mva = fTMVAReader_[2]->EvaluateMVA( methodName_ );
    else  
      mva = fTMVAReader_[6]->EvaluateMVA( methodName_ );
