		   ...
		   )
{
  ...
  TFile *fSignal = new TFile(Form("/data_CMS/cms/ivo/AntiEMVA/Trees/MVAOutput/MvaOutput_AntiEMVA_v3%s_Signal.root",Discr_.data()),"READ"); 
  if(fSignal->IsZombie()){
    cout << "No such file!" << endl;
    return;
  }
  TFile *fBackgrd = new TFile(Form("/data_CMS/cms/ivo/AntiEMVA/Trees/MVAOutput/MvaOutput_AntiEMVA_v3%s_Backgrd.root",Discr_.data()),"READ"); 
  ...
  for ( std::vector<std::string>::const_iterator category = categories.begin();
    ...
    string outputName2 = Form("plots/plotMVAOutput_v3%s_%s_%s",Discr_.data(),category->data(),Region.data());
    c1->Print(std::string(outputName2).append(".png").data());
    c1->Print(std::string(outputName2).append(".pdf").data());
  }
  fSignal->Close();
